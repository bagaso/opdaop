                                ...
                                @foreach($ticket->replies as $reply)
                                    <div class="direct-chat-msg {{ $ticket->user_id === $reply->user_id  ? 'right' : 'left'}}">
                                        <div class="direct-chat-info clearfix">
                                            <span class="direct-chat-name pull-{{ $ticket->user_id === $reply->user_id  ? 'right' : 'left'}}">{{ $reply->user->username <> '###' ? (!auth()->user()->isAdmin() && $reply->user->isAdmin()  ? $reply->user->fullname : $reply->user->username) : $reply->user->username }} - <small class="label label-{{ $reply->user->username <> '###' ? $reply->user->group->class :  'warning' }}">{{ $reply->user->username <> '###' ? $reply->user->group->name : 'Deleted' }}</small></span>
