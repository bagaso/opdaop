...
use App\Http\Requests\ManageUsers\UserListSubResellerSearchRequest;
use App\User;
use Carbon\Carbon;
...
class UserListSubResellerController extends Controller
{
    /**
    ...
    {
        $this->middleware(['auth', 'access_page.manage_users::user_list_sub_reseller']);
    }
    ...
    {
        return view('theme.default.manage_users.user_list_sub_reseller');
    }

    public function user_list(UserListSubResellerSearchRequest $request)
    {
        $query = User::SubResellers(auth()->user())->with('group', 'subscription', 'status', 'upline')->selectRaw('users.id, users.username, users.email, users.group_id, users.subscription_id, users.status_id, users.freeze_mode, users.credits, users.expired_at, users.parent_id, users.created_at');
        return datatables()->eloquent($query)
            ...
            ->filterColumn('upline.username', function ($query, $keyword) {
