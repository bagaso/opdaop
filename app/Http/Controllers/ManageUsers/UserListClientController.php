...
use App\Http\Requests\ManageUsers\UserListClientSearchRequest;
...
class UserListClientController extends Controller
{
    /**
    ...
    {
        $this->middleware(['auth', 'access_page.manage_users::user_list_client']);
    }
    ...
    {
        return view('theme.default.manage_users.user_list_client');
    }

    public function user_list(UserListClientSearchRequest $request)
    {
        $query = User::Clients(auth()->user())->with('group', 'subscription', 'status', 'upline')->selectRaw('users.id, users.username, users.email, users.group_id, users.subscription_id, users.status_id, users.freeze_mode, users.credits, users.expired_at, users.parent_id, users.created_at');
