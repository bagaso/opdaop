...
use App\Http\Requests\ManageUsers\UserDownlineRequest;
use App\User;
use Carbon\Carbon;
...
class UserDownlineController extends Controller
{
    ...
    {
        $this->middleware(['auth', 'access_page.manage_users:user_downline']);
    }
    ...
    public function index($id = 0)
    {
        ...
        return view('theme.default.manage_users.user_downline', compact('user'));
    }

    public function user_list(UserDownlineRequest $request, $id = 0)
    {
        $user = User::findorfail($id);
        $query = User::DownLines($user)->with('group', 'subscription', 'status', 'upline')->selectRaw('users.id, users.username, users.email, users.group_id, users.subscription_id, users.status_id, users.freeze_mode, users.credits, users.expired_at, users.parent_id, users.created_at');
