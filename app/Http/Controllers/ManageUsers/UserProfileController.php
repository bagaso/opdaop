...
class UserProfileController extends Controller
{
    ...
    public function update(UserProfileRequest $request, $id = 0)
    {
        ...
        $user->expired_at = $user->paidSubscription() ? ((int)$request->subscription <> $user->subscription_id ? $date_now->copy()->addSeconds(($date_now->diffInSeconds($expired_at) * intval($user->subscription->cost)) / intval($user_subscription->cost)) : $expired_at) : $expired_at;
        $user->save();
